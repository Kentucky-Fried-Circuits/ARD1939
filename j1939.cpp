...
byte ARD1939::Init(int nSystemTime)
{
  ...
  j1939Stat.re_nSourceAddress = NULLADDRESS;
...
byte ARD1939::Operate(byte* nMsgId, long* lPGN, byte* pMsg, int* nMsgLen, byte* nDestAddr, byte* nSrcAddr, byte* nPriority)
{
  ...
  *nMsgId = re_Receive(lPGN, &pMsg[0], nMsgLen, nDestAddr, nSrcAddr, nPriority);
  ...
  {
    if(*nDestAddr != j1939Stat.re_nSourceAddress
  ...
  if(j1939Stat.canOperateMaybe == true)
  {
    ...
    switch(*lPGN)
    {
      case PGN_REQUEST:
        if(*nMsgId == J1939_MSG_PROTOCOL)
        {
          ...
          && pMsg[2] == CANNOT_CLAIM_SA_LSB)
          {
            ...
            {
              Transmit(d28, re_PGN_ADDRESS_CLAIMED, j1939Stat.re_nSourceAddress, GLOBALADDRESS,
                            &j1939Name[NAME_ID], 8);
            }
            else if(*nDestAddr == j1939Stat.re_nSourceAddress)
            {
              Transmit(d28, re_PGN_ADDRESS_CLAIMED, j1939Stat.re_nSourceAddress, *nSrcAddr,
                            &j1939Name[NAME_ID], 8);
            }
          }
        }
      ...
      case re_PGN_ADDRESS_CLAIMED:
        if(*nSrcAddr == j1939Stat.re_nSourceAddress) // we received a message from our address. We need to tell them to try a different address by sending our own Address Claimed message
        {
  ...
  *re_nPriority = 0;
  if(canReceive(&re_lID, &re_pMsg[0], re_nMsgLen, _CAN0) == 0) // we have a message
  {
    ...
    *re_nDestAddr = GLOBALADDRESS;
    if(re_IsPeerToPeer(*re_lPGN) == true)
    {
    ...
    if(re_isTransportProtocol(re_lPGN, &re_pMsg[0]) == true)
...
byte ARD1939::Transmit(byte re_nPriority, long re_lPGN, byte re_nSourceAddress, byte re_nDestAddress, byte* re_pData, int re_nDataLen)
{
  long re_lID;
  if(re_nDataLen > J1939_MSGLEN)
    return ERR;
  re_lID = ((long)re_nPriority << 26) + (re_lPGN << 8) + (long)re_nSourceAddress;
  if(re_IsPeerToPeer(re_lPGN) == true)
    re_lID = re_lID | ((long)re_nDestAddress << 8);
  if(re_nDataLen > 8)
#if TRANSPORT_PROTOCOL_XMIT == 1
    return re_RTSCTSTransmit(re_nPriority, re_lPGN, re_nSourceAddress, re_nDestAddress, re_pData, re_nDataLen);
#else
    return ERR;
#endif
  else
...
bool ARD1939::re_IsPeerToPeer(long re_lPGN)
{
...
{
   return j1939Stat.re_nSourceAddress;
}
...
byte ARD1939::SetMessageFilter(long re_lPGN)
{
  ...
  if((re_lPGN & 0x00FF00) == PGN_REQUEST)
	  re_lPGN = PGN_REQUEST;
  if(re_isFilterActive(re_lPGN) == true)
    re_nResult = OK;
  else
  {
    for(re_i = 0; re_i < MSGFILTERS; re_i++)
...
bool ARD1939::re_isFilterActive(long re_lPGN)
{
  bool re_bResult;
  int re_i;
  re_bResult = false;
...
//            v63[re_i+1] = v33.re_pFullMessage[nPointer + re_i];
  ...
  if(re_lPGN == TPCM_PGN && re_pMsg[0] == d04 && v33.v20 == d01
  && v33.v32 == false)
  {    
      v33.re_lPGN = (((long)re_pMsg[7]) << 16) + (((long)re_pMsg[6]) << 8) + (long)re_pMsg[5];
      if(re_isFilterActive(v33.re_lPGN) == true)
      {
...
//            v63[re_i+1] = v34.re_pFullMessage[nPointer + re_i];
  ...
  if(re_lPGN == TPCM_PGN && re_nDestAddr == j1939Stat.re_nSourceAddress && re_pMsg[0] == TPCM_RTS_CONTROL_BYTE)
  {
    int v77;
...
#if TRANSPORT_PROTOCOL_XMIT == 1
byte ARD1939::re_RTSCTSTransmit(byte re_nPriority, long re_lPGN, byte re_nSourceAddress, byte re_nDestAddress, byte* re_pData, int re_nDataLen) // re_DataLen was v77
{
  UNUSED(re_nPriority);
...
//    for(re_i = 0; re_i < re_nDataLen; re_i++) // TEST
 //     re_j1939bam->re_pFullMessage[re_i] = re_pData[re_i]; //copy the data to re_j1939bam
    ...
    re_j1939bam->v26 = re_nSourceAddress;
